...
Parameters:
    ...
    Input, double A, the matrix in lower-triangular form.

    Input/output, double Y, the estimate for the eigenvector.

    Input, int maxiter, the maximum number of iterations allowed.

    Input, double eps, an error tolerance.
...
SIZE_T first_eigen(SIZE_T p, double *A, double *Y, SIZE_T maxiter, double eps,
		double *lambda)
{
	double *AY, norm, dif = 1e+100, *Ycurr, lambdacurr;
//	double siny1y2
	SIZE_T i, iter;
...
//		siny1y2 = sqrt(1 - SQ(vecxvec (Y, p, Ycurr)));
//		printf("%e \n", siny1y2);
	}
...
/*	for (i = 0; i < p * (p + 1)/2; i++)
	...
	if (k == 1)  
		ind = first_eigen(p, A, y, maxiter, eps, &evals[0]);
	else 	{
		double *a;
...
/*		for (i = 0; i < p * (p + 1)/2; i++)
		...
		ind = first_eigen(p, a, y, maxiter, eps, &evals[k-1]);
		    ...
		    SIZE_T maxiter, double eps, double *evals)
{
	int i;
	for (i = 1; i <= k; i++)
		kth_eigen(p, i, A, evecs, maxiter, eps, evals);
}
			 ...
			 double *evals)
{
	...
	for (rankmatrix = 0; ((proptr <= 0.995) && (rankmatrix < MIN(p, rankmax))) ; rankmatrix++) {  
		...
		if (kth_eigen(p, rankmatrix + 1, A, evecs, 100000, 1e-10, evals))
...
SIZE_T symm_Moore_Penrose_inverse(SIZE_T p, SIZE_T rankmax, double *A, 
				  double *Ainv)
{
	/* This function calculates the Moore_Penrose_inverse of a symmetric
	   p x p matrix A, or maximum rank rankmax stored in lower triangular 
	   format. */
	double **Evecs, *evals;
	SIZE_T i, j, k, rnk;
	
	MAKE_MATRIX(Evecs, p, rankmax);
	MAKE_VECTOR(evals, p);

	rnk = first_rank_eigens(p, rankmax, A, Evecs, evals);
	...
	for (i = 0; i < p; i++)
		for (j = 0; j <= i; j++)
			for (k = 0; k < rnk; k++)
				Ainv[LTINDEX(i, j)] += Evecs[i][k] * Evecs[j][k] / evals[k];
	FREE_MATRIX(Evecs);
	FREE_VECTOR(evals);
	return rnk;
}
